...
from PetaSAN.core.common.CustomException import DiskListException, CephException, MetadataException
...
def get_disks_meta_list(args):
    ...
    try:
        ...
        if e.id == CephException.GENERAL_EXCEPTION:
            print("Error : {} , Cannot get disk meta.".format(e.message))
            sys.exit(-1)
...
def update_replication_info(args):
    ...
    try:
        ...
        confirm = replication_handler.update_replication_info(disks_metadata, new_replication_info)
        ...
        print("Error : Cannot update replication info on destination disk " + disk_id + " , Exception , {}".format(e.message))
        sys.exit(-1)
...
def delete_replication_info(args):
    ...
    try:
        ...
        print("Error : Cannot delete replication info on destination disk " + disk_id + " , Exception , {}".format(e.message))
        sys.exit(-1)
...
def get_disk_snapshots(args):
    ...
    try:
        ...
        if e.id == CephException.GENERAL_EXCEPTION:
            print("Error : {} , Cannot get disk {} snapshots.".format(e.message, disk_id))
            sys.exit(-1)
...
def delete_snapshot(args):
    ...
    try:
        ...
        if e.id == CephException.GENERAL_EXCEPTION:
            print("Error : " + str(e.message) + " , Cannot delete snapshot \"" + snap_name + "\" of disk " + disk_id)
            sys.exit(-1)
...
def rollback_snapshot(args):
    ...
    try:
        ...
        if e.id == CephException.GENERAL_EXCEPTION:
            print("Error : " + str(e.message) + " , Cannot rollback disk " + disk_id + " to snapshot " + snap_name)
            sys.exit(-1)
...
def delete_snapshots(args):
    ...
    try:
        ...
        if e.id == CephException.GENERAL_EXCEPTION:
            print("Error : " + str(e.message) + " , Cannot delete all snapshots of disk " + disk_id)
            sys.exit(-1)
...
def run_replication_job(args):
    ...
    try:
        ...
        text = "{} - Job {} | Error : {}.".format(system_date_time, str(job_id), e.message)
...
def get_all_images(args):
    try:
        ...
        if e.id == CephException.GENERAL_EXCEPTION:
            print("Error : {} , Cannot get images list.".format(e.message))
            sys.exit(-1)
