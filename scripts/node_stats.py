...
def get_stats():
    sar_result = Sar(True).run(SAMPLE_DURATION)
    _get_cpu(sar_result)
    _get_ram(sar_result)
...
def _get_disk(sar_result):
    for disk in sar_result.disks:
        ...
        val = float(disk.__dict__[SarOutputProperties.disk_wr_sec]) * 512.00
        _send_graphite(path_key, val)

        path_key = "disks.throughput.{}_read".format(disk.__dict__[SarOutputProperties.disk_DEV])
        val = float(disk.__dict__[SarOutputProperties.disk_rd_sec]) * 512.00
        _send_graphite(path_key, val)
