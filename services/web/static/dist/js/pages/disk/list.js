        ...
        success: function (data) {
            ...
            $("tr.disk").each(function () {
                for (var key in diskData) {
                    ...
                    status = parseInt($this.find("input.status").val());
                    ...
                    if (key == id && diskData[key] != status) {
                        if (diskData[key] == 1) {
                            ...
                            $('#actionID_' + id).html("<div title='Stop' class='btn-group'><form action=" + url_action + " method='post'> " +
                                "<button type='submit'   class='btn btn-default'><i class='fa fa-stop'></i> </button> </form></div>");
                        }
                        else if (diskData[key] == 2 && delete_job_id <= 0) {
                            ...
                            $('#actionID_' + id).html("<div title='Start' class='btn-group' style='padding-right: 4px;'><form action= " + url_action_start + " method='post'> " +
                                "<button type='submit' class='btn btn-default' onclick=\"return doStart('" + id + "','" + name + "','" + pool + "');\"><i class='fa fa-play'></i> </button> </form></div>" +
                                "<div title='Edit'class='btn-group' style='padding-right: 4px;'><form action=" + url_action_edit + " method='post'> " +
                                "<button type='submit' class='btn btn-default'><i class='fa fa-edit'></i> </button> </form></div>" +
                                "<div title='Delete' class='btn-group' style='padding-right: 4px;'><form action=" + url_action_remove + " method='post'> " +
                                "<button type='submit' class='btn btn-default' onclick=\"return doDelete('" + name + "');\"><i class='fa fa-remove'></i> </button> </form></div>" +
                                "<div title='Detach' class='btn-group'><form action=" + url_action_detach + " method='post'> " +
                            ...
                            $('#statusID_' + id).html(" <span class='badge bg-unAttached' style='padding-right: 4px;'>Detached</span>" +
                                "<input type='hidden' class='status' id='disk_status_" + id + "' value='" + status + "'>");
                            $('#actionID_' + id).html("<div title='Attach' class='btn-group'><form action=" + url_action + " method='post'> " +
                                "<button type='submit'   class='btn btn-default'><i class='fa fa-chain'></i> </button> </form></div>" +
                                "<div title='Delete' class='btn-group'><form action=" + url_action_remove + " method='post'> " +
                                "<button type='submit' class='btn btn-default' onclick=\"return doDelete('" + name + "');\"><i class='fa fa-remove'></i> </button> </form></div>");
                        }
